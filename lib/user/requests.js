    ...
    sofaInternals.request.register(internals.requestGroupName)
        .requests([
...

            {
                ...
                group: internals.requestGroupName,
                comment: 'destroy user document. \n' +
                         'no _rev id required to execute destroy. \n' +
                         'normally couchdb requires _rev id to destory document. \n'+
                         'This function gets _rev id as a convenience. \n' +
                         'returns error if the document does not exist.',
...

            {
                name: 'findById',
                group: internals.requestGroupName,
                comment: 'search for user record id. \n' +
                         'all ids in couchdb are unique.',
                handler: function (uniqueId, callback) {

                    sofaInternals.db.view('user', 'findById', { keys: [uniqueId] }, function (err, result) {

                        if (err) {

                            errors.findByIdFailed.nanoMessage = err;
                            return callback(errors.findByIdFailed, null);
                        }
