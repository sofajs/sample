        ...
        .tools([
...

            {
                ...
                comment: 'generate unique \'**username**\' and \'**email**\'. \n' +
                         ...
                         '* **err**  \n' +
                         '  - null if no errors. \n' +
                         '  - err exists returns one of the following: \n' +
                ...
                handler: function (username, email, callback) {
                    ...
                    return sofaInternals.foundation.core.uniqueCreate(username, 'username', function (err, docId, docRev) {

                        if (err) {
                            if (err === 'uuid already exists.') {
                                return callback('username already exists.', null, null);
                            }
                            return ('couchdb request failed.', null, null);
                        }

                        var usernameConfirmation = { id: docId, rev: docRev };

                        return sofaInternals.foundation.core.uniqueCreate(email, 'useremail', function (err, emailDocId, emailDocRev) {

                            if (err) {

                                if (err === 'uuid already exists.') {

                                    sofaInternals.tools.user.rollbackOne(usernameConfirmation.id, function (err, result) {

                                        if (err) {

                                            // username to be rolled back because email is not unique.
                                            // Howeve, rollback queried failed b/c of technical issue.
                                            return ('username rollback failed.', null, null);
                                        }

                                        // console.log('rollbackOne succeeded' + JSON.stringify(result));
            ...
            // @todo change to rollbackBoth and rollbackOne
            // uniqueDestroy should solve one rollback.
            // rollbackUniqueValues
...

            {
                ...
                group: internals.toolGroup,
                comment: 'rollback both values for \'**username**\' and \'**email**\' \n' +
                         'or other unique values if needed. \n' +
                         'Example use: cleanup unique username and email records when requests.user.create fails.\n' +
                         '\n' +
                         '* errMessage original error message that caused the rollback.' +
                         '* arrayOfIdsToDestroy' +
                         '**callback(err, result)**\n' +
                         '* err \n' +
                         '  - original error message sent in errorMessage param.\n' +
                         '  - \'rollback failed.\' upon failure inside this function.\n',

                handler: function (usernameId, emailId, callback) {

                    return sofaInternals.foundation.core.uniqueDestroy(usernameId, function (err, result1) {

                        if (err) {
                            return callback('rollback failed.');
                        }

                        return sofaInternals.foundation.core.uniqueDestroy(emailId, function (err, result2) {
