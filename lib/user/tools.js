        ...
        .tools([
...

            {
                ...
                comment: 'make new uniqueUsername document. \n' +
                         '### Why a uniqueUsername? \n' +
                         'A uniqueUsername document is required because couchdb only \n' +
                         'enforces uniqueness on document uuids. So, to enforce unique usernames \n' +
                         'and emails in all user documents we create a document with an uuid of each username, \n' +
                         'and email which will enforce no duplicate username or email records. \n' +
                         'The uniqueUsername & uniqueEmail records only exists to enforce uniqueness. \n' +
                         'And, the functions labeled with \'unique\' in the front manage these unique docuements.' +
                         '\n' +
...

            {
                ...
                handler: function (usernameOriginal, usernameNew, callback) {
                    ...
                    return sofaInternals.tools.user.uniqueUsernameCreate(usernameNew, function (err, docId, docRev) {

                        var newUniqueUsername = docId;
                        var newUniqueUsernameRev = docRev;
                        ...
                        return sofaInternals.tools.user.uniqueUsernameDestroy('username/' + usernameOriginal, function (err, result) {
                            ...
                            if (err) {
                                ...
                                return sofaInternals.tools.user.uniqueUsernameDestroy(newUniqueUsername,
                                    function (err, result2) {

                                        return callback('Reverted transaction. UniqueUsernameDestroy failed.', null);
                            ...
                            return callback(null, newUniqueUsername, newUniqueUsernameRev);
                        });
                    });
                }
            }
